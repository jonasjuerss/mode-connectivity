...
import viktor.torch_autoneb.models as models
...
def neb(previous_cycle_data, model: models.ModelWrapper, neb_config: config.NEBConfig) -> dict:
    ...
    # Model
    neb_mod = neb_model.NEB(model, start_path.cuda(), target_distances.cuda())
    ...
    # Optimise
    for i in helper.pbar(range(optim_config.nsteps), "NEB"):
        print(i)
